...
static int vmac_rst_rc_en = 0;
struct work_struct detect_ep_rst_work;

static ssize_t vmac_reset_get(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
...
static void qdpc_mode_led(struct net_device *ndev, MODE_LED_OPS op)
{
	...
	case MODE_LED_INIT:
		if (gpio_request(MODE_LED_GPIO, ndev->name) < 0)
			printk(KERN_INFO
			       "%s: Failed to request GPIO%d for GPIO reset\n",
	...
	case MODE_LED_FLASH:
		if (inited)
			gpio_set_value(MODE_LED_GPIO,
				       ~gpio_get_value(MODE_LED_GPIO) & 0x01);

		break;
	}
}

static void qdpc_wps_button_event_wakeup(struct net_device *ndev,
					 WPS_Button_Event event)
{
	...
	if (event == WPS_BUTTON_DBGDUMP_EVENT)
				    ...
				    struct device_attribute *attr, char *buff)
{
	...
	wait_event_interruptible(WPS_Button_WaitQ,
				 WPS_BUTTON_VALID(wps_button_event));
...
static void qdpc_wps_polling_button_notifier(unsigned long data)
{
	...
	if ((current_level != QDPC_WPS_BUTTON_ACTIVE_LEVEL)
		...
		if ((jiffies - qdpc_wps_button_down_jiffies) >= 10 * HZ) {
			...
			qdpc_wps_button_event_wakeup(dev,
						     WPS_BUTTON_DBGDUMP_EVENT);

			printk(KERN_INFO
			       "WPS: button long press polling at %u\n",
			...
			printk(KERN_INFO
			       "WPS: button short press polling at %u\n",
			       (unsigned int)jiffies);
		}
	...
	if (qdpc_wps_button_down_jiffies != 0
	    && ((jiffies - qdpc_wps_button_down_jiffies) >= 10 * HZ)) {
...
int qdpc_wps_button_init(struct net_device *dev)
{
	...
	init_timer_compat(&qdpc_wps_button_timer,
				qdpc_wps_polling_button_notifier,
				(unsigned long)dev, 0);
