...
{
#define	QCSAPI_SHELL_PATH_MAX_SIZE	256
	struct stat file_stat;
	char base_name[QCSAPI_SHELL_PATH_MAX_SIZE + 1] = {0};

	memset(&file_stat, 0, sizeof(file_stat));
	strncpy(base_name, basename((char *)file_path_name), sizeof(base_name) - 1);
	if (!stat(file_path_name, &file_stat) && (file_stat.st_mode & S_IFREG)) {
		...
		strcpy(connect_payload->image_name, base_name);
