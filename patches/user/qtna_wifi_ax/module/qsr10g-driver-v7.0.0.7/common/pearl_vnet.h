...
#define vmac_readl(addr)	(le32_to_cpu(readl(IO_ADDRESS(addr))))


#if defined(VMAC_64BIT)
typedef uint64_t vmac_addr_t;
#define vmac_writel(val, addr)	do { \
		writel(cpu_to_le32((uint32_t)(uintptr_t)val), (IO_ADDRESS(addr))); \
	} while (0)

#define vmac_writell_hi(val, addr)	do { \
		writel(cpu_to_le32((uint64_t)(uintptr_t)(val) >> 32), (IO_ADDRESS(addr))); \
	} while (0)
#define vmac_mergell(lo, hi)	((lo) | (((uint64_t)(hi)) << 32))
#else /* !VMAC_64BIT */
typedef uint32_t vmac_addr_t;
#define vmac_writel(val, addr)	do { \
		writel(cpu_to_le32(val), (IO_ADDRESS(addr))); \
	} while (0)
...
#define vmac_writew(val, addr) do{ \
		writel(cpu_to_le16((uint16_t)(uintptr_t)(val)), (IO_ADDRESS(addr))); \
	} while (0)
