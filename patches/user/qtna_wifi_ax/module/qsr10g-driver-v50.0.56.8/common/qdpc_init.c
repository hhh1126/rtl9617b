...
static void qdpc_pcie_remove(struct pci_dev *pdev)
{
	...
	writel(TOPAZ_SET_INT(1 << PEARL_LH_IPC_EP_REBOOT), vmp->ep_ipc_reg);
...
void qdpc_pcie_ep_tx_en(struct vmac_priv *priv, bool enable)
{
	...
	value = le32_to_cpu(readl(PCIE_HDP_TX_DMA_CTRL(priv->pcie_reg_base)));
	...
	writel(cpu_to_le32(value), PCIE_HDP_TX_DMA_CTRL(priv->pcie_reg_base));
}
...
static int qdpc_ep_cfg_push_monitor(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		schedule_timeout_uninterruptible(msecs_to_jiffies(QDPC_EP_CFG_CHECK_PERIOD_MS));
	}
...
int qdpc_bringup_fw(struct vmac_priv *priv)
{
	...
	if (qdpc_pci_readl(&bda->bda_ep_state) & QDPC_EP_HAS_FRIMWARE) {
