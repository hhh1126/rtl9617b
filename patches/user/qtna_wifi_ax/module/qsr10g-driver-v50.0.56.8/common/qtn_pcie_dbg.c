...
static int vmac_read_mem(struct vmac_priv *vmp)
{
	...
	for (i = 0; i < vmac_dbg_show_num; i++) {
		...
		dbg_str(dbg, " %08x", vmac_readl(addr));
...
#define PEARL_HBM_CACHE_SIZE		(1 << 6)
	...
	wr_ptr = le32_to_cpu(readl(PCIE_HHBM_Q_WR_REG(vmp->pcie_reg_base)));
	rd_ptr = le32_to_cpu(readl(PCIE_HHBM_Q_RD_REG(vmp->pcie_reg_base)));
	max_pool_count = le32_to_cpu(readl(PCIE_HHBM_Q_LIMIT_REG(vmp->pcie_reg_base))) & HHBM_RDWR_PTR_MASK;
