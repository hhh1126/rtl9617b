				   ...
				   uint16_t len, const void *vdata)
{
	...
	union pearl_tqe_cpuif_descr desc;
	...
	if (is_multicast_ether_addr(eth->h_dest)) {
		...
		desc.data.buff_ptr_offset =
		    topaz_hbm_payload_buff_ptr_offset_bus((void *)bdata, pool,
							  NULL);
...
void vmac_set_dma_threshold(struct vmac_priv *vmp, uint32_t dma_threshold)
{
	...
	temp = readl(PCIE_HDP_AXI_CTRL(vmp->pcie_reg_base));
	...
	writel(temp, PCIE_HDP_AXI_CTRL(vmp->pcie_reg_base));
...
#ifdef QTN_RC_ENABLE_HDP
	ndev->if_port =  qtn_pcie_tqe_port_get();
#endif
