...
#ifdef CONFIG_PEARL_PCIE_HOST
			/* FIXME: MASK bit has been reverted in logic, fix it later */
			pci_write_config_word(pdev, 0x60, 0);
#endif
			PRINT_INFO("PCIe MSI Interrupt Enabled\n");
			priv->msi_enabled = 1;
		} else {
			PRINT_ERROR
			    ("PCIe MSI Interrupt enabling failed. Fall back to Legacy IRQ\n");
		}
	}
...
static bool qdpc_bar_check(struct vmac_priv *priv, qdpc_bar_t * bar)
{
	...
	if (busaddr) {		/*initialized BAR */
		resource_size_t bar_start =
		    pci_resource_start(priv->pdev, index);
		resource_size_t bar_end = pci_resource_end(priv->pdev, index);

		if (!bar_start) {
			printk(KERN_ERR "Invalid BAR address: %llx.\n",
		...
		if ((busaddr - offset) != bar_start) {
			printk(KERN_ERR
			       "Invalid BAR offset:%llx. BAR starts at %llx\n",
			       (unsigned long long)(busaddr - offset),
			       (unsigned long long)bar_start);
			return 0;
		}
		...
		if ((busaddr + len - 1) > bar_end) {
			printk(KERN_ERR
			       "Invalid BAR end address:%llx. BAR ends at 0x%llx\n",
			       (unsigned long long)(busaddr + len),
			       (unsigned long long)bar_end);
			return 0;
		}
		...
		if ((bar_start + offset) > bar_end) {
			printk(KERN_ERR
			       "Invalid BAR offset %llx, extends beyond end of BAR(%llx).\n",
			       (unsigned long long)(bar_start + offset),
			       (unsigned long long)bar_end);
			return 0;
		}

		/* Checks that mapped area is within bar */
		if ((bar_start + len + offset - 1) > bar_end) {
			printk(KERN_ERR
			       "Mapped area %llx, extends beyond end of BAR(%llx).\n",
			       (unsigned long long)(bar_start + len + offset -
						    1),
			       (unsigned long long)bar_end);
			return 0;
		}
				...
				uint8_t index, size_t len, uint32_t offset)
{
	...
	if (!request_mem_region(busaddr, len, QDPC_DEV_NAME)) {
		printk(KERN_ERR "Failed to reserve %u bytes of PCIe memory "
		       "region starting at %llx\n", (uint32_t) len,
	...
	if (!vaddr) {
		printk(KERN_ERR
		       "Failed to map %u bytes at BAR%u at bus address %llx.\n",
		       (uint32_t) len, index, (unsigned long long)busaddr);
	...
	printk(KERN_INFO "BAR:%u vaddr=0x%p busaddr=%llx offset=%u len=%u\n",
	       bar->b_index, bar->b_vaddr, (unsigned long long)bar->b_busaddr,
...
static void qdpc_map_sysctl_regs(struct vmac_priv *priv)
{
	...
	qdpc_map_bar(priv, &priv->sysctl_bar, QDPC_SYSCTL_BAR,
		     pci_resource_len(priv->pdev, QDPC_SYSCTL_BAR), 0);
}

static void qdpc_map_dma_regs(struct vmac_priv *priv)
{
	printk("%s() Mapping dma registers\n", __FUNCTION__);
	qdpc_map_bar(priv, &priv->dmareg_bar, QDPC_DMA_BAR,
		     pci_resource_len(priv->pdev, QDPC_DMA_BAR), 0);
}
...
int32_t qdpc_set_dma_mask(struct vmac_priv *priv)
{
	...
	unsigned long long dma_mask = DMA_BIT_MASK(64);
#else
	...
	if (!result) {
		...
		if (result) {
			printk
			    (" pci_set_consistent_dma_mask() error %d. Mask:0x%llx\n",
			     result, dma_mask);
			return 1;
		}
	} else {
...
{
	void *basereg =
	    QDPC_BAR_VADDR(priv->sysctl_bar, TOPAZ_PCIE_CFG0_OFFSET);

	qdpc_pcie_posted_write(priv->ep_pciecfg0_val & ~TOPAZ_ASSERT_INTX,
			       basereg);
}
