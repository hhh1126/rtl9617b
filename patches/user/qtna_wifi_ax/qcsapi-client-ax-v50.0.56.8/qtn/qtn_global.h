...
/*
 * Default QTN_GLOBAL_INIT_SELECT_GI_ENABLE values are:
 * 0 No GI sampling at all		QTN_SEL_GI_DISABLE
 * 1 Sample all clients			QTN_SEL_GI_ENABLE_ALL
 * 2 Sample non QTN clients		QTN_SEL_GI_ENABLE_NONQTN
 * 5 Sample all non-11ac clients	QTN_SEL_GI_ENABLE_NON_11AC	
 * 6 Sample non QTN, non 11ac clients	QTN_SEL_GI_ENABLE_NON_QTN_NON_11AC
 * 16 or above enable debug		QTN_SEL_GI_DEBUG_ENABLE
 */
#define QTN_GLOBAL_INIT_SELECT_GI_ENABLE	6
...
struct qtn_global_param {
	...
	uint8_t		g_gi_adapt_enable;
	uint8_t		g_pppc_adapt_enable;
	uint8_t		g_pppc_ss_iter;
