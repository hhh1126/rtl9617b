...
#define QVLAN_VID_MAX			4095
#define QVLAN_VID_MAX_S			12
#define QVLAN_VID_ALL			0xffff

#define IEEE80211_VLAN_ETHERTYPE_MAX	16
#define IEEE80211_VLAN_ETHERTYPE_ALL	0xFFFF
...
struct qtn_vlan_config {
	...
	union {
		struct vlan_dev_config {
			...
			uint32_t	prio_bitmap[howmany(QVLAN_VID_MAX, NBDW)];
		} dev_config;
		uint32_t	tagrx_config[howmany(QVLAN_VID_MAX * QVLAN_TAGRX_BITWIDTH, NBDW)];
	} u;
#define QVLAN_DEV_F_DROPCTAGGED	BIT(1)
	uint16_t flags;
	uint16_t vlan_ethertype_count;
	uint16_t vlan_ethertype[IEEE80211_VLAN_ETHERTYPE_MAX];
};
...
static inline
#endif
void qtn_vlan_config_htonl(struct qtn_vlan_config *vcfg, int tagrx)
{
	unsigned int i;
	...
	if (tagrx) {
		...
		for (i = 0; i < ARRAY_SIZE(vcfg->u.dev_config.prio_bitmap); i++)
			vcfg->u.dev_config.prio_bitmap[i] = htonl(vcfg->u.dev_config.prio_bitmap[i]);

		vcfg->vlan_ethertype_count = htons(vcfg->vlan_ethertype_count);
		for (i = 0; i < ARRAY_SIZE(vcfg->vlan_ethertype); i++)
			vcfg->vlan_ethertype[i] = htons(vcfg->vlan_ethertype[i]);
		vcfg->flags = htons(vcfg->flags);
	}
}
...
static inline
#endif
void qtn_vlan_config_ntohl(struct qtn_vlan_config *vcfg, int tagrx)
{
	unsigned int i;
	...
	if (tagrx) {
		...
		for (i = 0; i < ARRAY_SIZE(vcfg->u.dev_config.prio_bitmap); i++)
			vcfg->u.dev_config.prio_bitmap[i] = ntohl(vcfg->u.dev_config.prio_bitmap[i]);
		vcfg->vlan_ethertype_count = htons(vcfg->vlan_ethertype_count);

		for (i = 0; i < ARRAY_SIZE(vcfg->vlan_ethertype); i++)
			vcfg->vlan_ethertype[i] = htons(vcfg->vlan_ethertype[i]);
		vcfg->flags = ntohs(vcfg->flags);
	}
}
